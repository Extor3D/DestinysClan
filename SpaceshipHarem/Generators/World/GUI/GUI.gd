...
onready var colorbutton_scene = preload("res://Generators/World/GUI/ColorPickerButton.tscn")


const GIFExporter = preload("res://Generators/World/addons/gdgifexporter/exporter.gd")
const MedianCutQuantization = preload("res://Generators/World/addons/gdgifexporter/quantization/median_cut.gd")
...
func export_gif(frames, frame_delay, progressbar):
	...
	if OS.get_name() != "HTML5" or !OS.has_feature('JavaScript'):
		...
		if OS.get_name() == "OSX":
			...
			file.open("res://Generators/World/%s.gif"%String(sd), File.WRITE)
