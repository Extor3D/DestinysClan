...
func _ready():
	rng.randomize()
	var weapon1 = rng.randi_range(1, SpreadWeapon.weapons.size())
	add_weapon(weapon1, difficulty)
	var weapon2 = rng.randi_range(1, SpreadWeapon.weapons.size())
	while weapon1 == weapon2:
		weapon2 = rng.randi_range(1, SpreadWeapon.weapons.size())
	add_weapon(weapon2, difficulty)
	if difficulty > 5:
		var weapon3 = rng.randi_range(1, SpreadWeapon.weapons.size())
		while weapon1 == weapon3 or weapon2 == weapon3:
			weapon3 = rng.randi_range(1, SpreadWeapon.weapons.size())
