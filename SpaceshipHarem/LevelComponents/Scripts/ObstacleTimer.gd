...
export (float) var tunnel_size = 200
export (float) var min_tunnel_size = 150
export (float) var max_tunnel_size = 300
export (float) var top = 100
export (float) var bot = 260
...
func spawn_tunnel():
	...
	var next_y = clamp(current_tunnel_y + rand, top, bot)
	...
	for i in 8:
		spawn_wall(Vector2(800, current_tunnel_y - tunnel_size/2 - i * 30), rot)
		spawn_wall(Vector2(800, current_tunnel_y + tunnel_size/2 + i * 30), rot+PI)
	
	#Set current Y for next tunnel piece
	current_tunnel_y = next_y
	
	#Change the spacing between walls for the next piece
	tunnel_size = clamp(tunnel_size + rng.randi_range(-50, 50), min_tunnel_size, max_tunnel_size)
